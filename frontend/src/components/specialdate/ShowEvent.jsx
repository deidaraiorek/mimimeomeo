...
const ShowEvent = ({ events, deleteEvent }) => {
    return (
        <div className="show-event-container">
            {events.length > 0 ? (
                <div className="flex flex-col gap-4 m-3">
                    {events.map((event) => (
                        <div key={event.id} className="flex p-4 shadow-xl bg-white hover:shadow-2xl transition-shadow duration-300 rounded-xl">
                            <div className="flex flex-col items-center text-purple-900  mr-7 justify-between">
                                ...
                                <button className="bg-purple-300 text-white p-2 rounded-lg hover:bg-purple-400 font-bold" onClick={() => deleteEvent(event.id)}>Complete</button>
                            </div>
                            <div>
